...
//		node.setPathUptoPreviousNode(null);
	...
	public void ccrpPlusPlus()
	{
		...
		System.out.println("Egress Time : " + pathList.get(pathList.size()-1)
				.getArrivalTime().get(pathList.get(pathList.size()-1).getRouteNodeList().size()-1));
...
//			v.setPathUptoPreviousNode(pathUptoPreviousNode);
	...
	public Route findRoute(Node destination)
	{
		...
		route.getDepartureTime().add(-1);
		do
		{
			Node parentNode = destination.getParent();
			route.getRouteNodeList().add(0,parentNode);
			route.getArrivalTime().add(0,destination.getTravelTime());
			
			Edge parentEdge = destination.getParentEdge();
			route.getRouteEdgeList().add(0,parentEdge);
			int depart = destination.getTravelTime() - parentEdge.getTravelTime();
			route.getDepartureTime().add(0,depart);
			destination = parentNode;
		}while(destination.getParent()!=null);
		route.getRouteNodeList().add(0,destination);
		route.getArrivalTime().add(0,0);
		return route;
	}
}
